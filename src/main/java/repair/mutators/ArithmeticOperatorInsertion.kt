...
class ArithmeticOperatorInsertion(val op: BinaryExpr.Operator? = null, val exprToAdd: Expression? = null): MutatorRepair<Expression>() {
    ...
    override fun checkedRepair(program: BuggyProgram, expr: Expression): List<BinaryExpr> {
        return if(op == null && exprToAdd == null){
            ...
            when(exprType.describe()){
                ...
                ).map { BinaryExpr(expr.clone(), IntegerLiteralExpr(it.first.toString()), it.second) }
                ...
                ).map { BinaryExpr(expr.clone(), LongLiteralExpr(it.first.toString()), it.second) }
                ...
                ).map { BinaryExpr(expr.clone(), DoubleLiteralExpr(it.first.toString()), it.second) }
